...
> <mark>확장성 있는 웹 애플리케이션</mark>과 <mark>안정적인 데이터 아키텍처</mark>를 설계하는 풀스택 개발자입니다.
...
**🏗️ 기술 및 아키텍처 도입 배경**
* **Next.js (App Router):** 리뷰 게시글의 <mark>검색 엔진 최적화(SEO)</mark>를 확보하고, 다수의 외부 음원 API를 <mark>서버사이드에서 병렬로 호출</mark>하여 클라이언트 렌더링 부하를 줄이기 위해 도입했습니다.
* **Supabase & PostgreSQL:** 실시간으로 유저의 평점과 피드가 업데이트되는 커뮤니티 특성상, <mark>Realtime Subscription</mark> 기능과 강력한 관계형 데이터베이스를 함께 활용하여 백엔드 개발 생산성을 극대화했습니다.
...
**🏗️ 기술 및 아키텍처 도입 배경**
* **React:** 퀴즈 진행 및 대전 중 페이지 새로고침이 발생하지 않도록 <mark>SPA(Single Page Application)</mark> 형태로 구성하여 앱과 같은 부드러운 사용자 경험(UX)을 제공하고자 했습니다.
* **Supabase Realtime:** 별도의 복잡한 WebSocket 서버를 구축하고 유지보수하는 대신, BaaS의 실시간 채널 기능을 활용하여 <mark>저지연(Low-latency) 1:1 매칭</mark>과 양방향 라이브 채팅을 신속하고 안정적으로 구현했습니다.
...
**🏗️ 기술 및 아키텍처 도입 배경**
* **Spring Boot & OracleDB:** 다수의 유저 및 스터디 그룹 간의 <mark>복잡한 권한 처리(접근 제어)</mark>와 일정, 파일 등의 관계형 데이터를 안정적이고 무결성 있게 다루기 위해 엔터프라이즈급 생태계인 Spring Boot 환경을 채택했습니다.
* **AWS S3:** 스터디원들이 공유하는 대용량 학습 자료 및 이미지 파일들을 웹 서버(WAS) 부하 없이 <mark>안전하게 분산 저장</mark>하고 로드하기 위해 클라우드 스토리지를 연동했습니다.
