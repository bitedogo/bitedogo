...
**💡 기획 의도**
해외의 'RateYourMusic'이나 'Pitchfork'처럼 리스너들이 참여하는 음악 아카이브 및 리뷰 플랫폼이 국내에는 대중화되지 않았고, 기존 국내 전문 평론 매체들의 활성도가 낮다는 점에 주목했습니다. 이에 국내 대중음악 리스너들이 자유롭게 앨범 평점을 남기고 깊이 있는 감상을 공유할 수 있는 **'사용자 참여형 음악 평론 커뮤니티 서비스'**를 목적으로 기획 및 개발했습니다.

**🏗️ 기술 및 아키텍처 도입 배경**
* **Next.js (App Router):** 리뷰 게시글의 검색 엔진 최적화(SEO)를 확보하고, 다수의 외부 음원 API를 서버사이드에서 병렬로 호출하여 클라이언트 렌더링 부하를 줄이기 위해 도입했습니다.
* **Supabase & PostgreSQL:** 실시간으로 유저의 평점과 피드가 업데이트되는 커뮤니티 특성상, Realtime Subscription 기능과 강력한 관계형 데이터베이스를 함께 활용하여 백엔드 개발 생산성을 극대화했습니다.
...
**💡 기획 의도**
딱딱하고 지루할 수 있는 프로그래밍 문법 학습에 **'게이미피케이션(Gamification)'** 요소를 결합하여 지속적인 학습 동기를 부여하고자 했습니다. 단순히 문제를 푸는 것을 넘어, 유저 간 실시간 1:1 대전과 업적 시스템을 통해 자바(Java)를 놀이처럼 재미있게 학습할 수 있는 생태계를 구축하는 것이 목적이었습니다.

**🏗️ 기술 및 아키텍처 도입 배경**
* **React:** 퀴즈 진행 및 대전 중 페이지 새로고침이 발생하지 않도록 SPA(Single Page Application) 형태로 구성하여 앱과 같은 부드러운 사용자 경험(UX)을 제공하고자 했습니다.
* **Supabase Realtime:** 별도의 복잡한 WebSocket 서버를 구축하고 유지보수하는 대신, BaaS의 실시간 채널 기능을 활용하여 저지연(Low-latency) 1:1 매칭과 양방향 라이브 채팅을 신속하고 안정적으로 구현했습니다.
...
**💡 기획 의도**
개발자나 취업 준비생들이 스터디를 운영할 때 소통은 메신저로, 자료는 클라우드로, 일정은 캘린더로 각각 파편화되어 진행되는 비효율에 주목했습니다. 팀 빌딩부터 목표 달성까지 필요한 모든 협업 기능을 한 곳에 모은 **'올인원 스터디 워크스페이스'**를 제공하여 학습과 협업의 마찰을 최소화하고자 기획했습니다.

**🏗️ 기술 및 아키텍처 도입 배경**
* **Spring Boot & OracleDB:** 다수의 유저 및 스터디 그룹 간의 복잡한 권한 처리(접근 제어)와 일정, 파일 등의 관계형 데이터를 안정적이고 무결성 있게 다루기 위해 엔터프라이즈급 생태계인 Spring Boot 환경을 채택했습니다.
* **AWS S3:** 스터디원들이 공유하는 대용량 학습 자료 및 이미지 파일들을 웹 서버(WAS) 부하 없이 안전하게 분산 저장하고 로드하기 위해 클라우드 스토리지를 연동했습니다.
